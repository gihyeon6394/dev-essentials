...
> - 비멱등 요청 <sub>e.g. POST</sub>은 파이프라인을 이용해선 안된다. <sub>에러 발생 시 파이프라인에 어떤 요청이 실패했는지 알 수 없음</sub>
...
- e.g. 지속 커넥션을 맺은 뒤 유휴 상황 <sub>클라이언트가 요청을 보내지 않음<sub>
...
- 비멱등 요청을 다시 보내야하는 상황이 벌어지면?
    ...
    - e.g. 브라우저는 캐시된 POST 요청에 대해 사용자에게 재요청 여부를 물음

> **멱등** <sup>idempotent</sup> : 실행횟수에 상관없이 결과가 같은 연산  
> e.g. GET, HEAD, PUT, DELETE, TRACE, OPTIONS
